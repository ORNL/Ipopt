...
/** pointer to MA57 function that can be set via KLUSolverInterface::SetFunctions() */
...
)
{
   ...
   roptions->AddBoundedNumberOption(
      ...
      1.0, true,
      1e-4,
      "Ipopt may increase pivtol as high as klu_pivtolmax to get a more accurate solution to the linear system.");
...
void KLUSolverInterface::SetFunctions(
...
)
{
   ...
   Index print_level;
   options.GetIntegerValue("klu_print_level", print_level, prefix);

   options.GetNumericValue("klu_pivtol", pivtol_, prefix);
   if( options.GetNumericValue("klu_pivtolmax", pivtolmax_, prefix) )
   {
   ...
   options.GetNumericValue("klu_pre_alloc", ma57_pre_alloc_, prefix);
   Index ma57_pivot_order;
   options.GetIntegerValue("klu_pivot_order", ma57_pivot_order, prefix);
   ...
   options.GetBoolValue("klu_automatic_scaling", ma57_automatic_scaling, prefix);

   // CET 04-29-2010
   Index ma57_block_size;
   options.GetIntegerValue("klu_block_size", ma57_block_size, prefix);

   Index ma57_node_amalgamation;
   options.GetIntegerValue("klu_node_amalgamation", ma57_node_amalgamation, prefix);

   Index ma57_small_pivot_flag;
   options.GetIntegerValue("klu_small_pivot_flag", ma57_small_pivot_flag, prefix);
   // CET 04-29-2010
   ...
   else
   {
      ASSERT_EXCEPTION(dim_ > 0 && nonzeros_ > 0, INVALID_WARMSTART,
                       "KLUSolverInterface called with warm_start_same_structure, "
...
ESymSolverStatus KLUSolverInterface::SymbolicFactorization(
   const Index* airn,
   const Index* ajcn
)
{
   DBG_START_METH("KLUSolverInterface::SymbolicFactorization", dbg_verbosity);
...
ESymSolverStatus KLUSolverInterface::Factorization(
   const Index* /*airn*/,
   const Index* /*ajcn*/,
   bool         check_NegEVals,
   Index        numberOfNegEVals
...
{
   DBG_START_METH("KLUSolverInterface::Factorization", dbg_verbosity);
   if( HaveIpData() )
   {
   ...
   {
      Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA,
                     "In KLUSolverInterface::Factorization: negevals_ = %" IPOPT_INDEX_FORMAT ", but numberOfNegEVals = %" IPOPT_INDEX_FORMAT "\n", negevals_, numberOfNegEVals);
...
ESymSolverStatus KLUSolverInterface::Backsolve(
   Index   nrhs,
   Number* rhs_vals
)
{
   DBG_START_METH("KLUSolverInterface::Backsolve", dbg_verbosity);
   if( HaveIpData() )
   {
...
{
   DBG_START_METH("KLUSolverInterface::NumberOfNegEVals", dbg_verbosity);
...
{
   DBG_START_METH("KLUSolverInterface::IncreaseQuality", dbg_verbosity);
